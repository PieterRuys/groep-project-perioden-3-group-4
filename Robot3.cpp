...
int main(){
  ...
  while(true){
    ...
    cout << colorval << endl;
    if (colorval >= 60){
      BP.set_motor_power(PORT_B, power+((colorval-80)/1.5));
      BP.set_motor_power(PORT_C, -((colorval-80)/1.5));
    }
    else if (colorval <= 40){
      BP.set_motor_power(PORT_B, -((40-colorval)/1.5));
      BP.set_motor_power(PORT_C, power+((40-colorval)/1.5));
    }
    else if (colorval > 60 && lightval > 40){
      BP.set_motor_power(PORT_B, 40);
      BP.set_motor_power(PORT_C, -40);
    }
      else if (colorval > 40 && lightval > 60){
      BP.set_motor_power(PORT_B, -40);
      BP.set_motor_power(PORT_C, 40);
    }
