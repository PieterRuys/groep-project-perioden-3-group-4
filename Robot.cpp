...
int main(){
  ...
  while(true){
    ...
    if (lightval <= 50){
      BP.set_motor_power(PORT_B, lightval*power/30);
      BP.set_motor_power(PORT_C, (power+((50-lightval)/30)));
    }
    if (lightval > 50){
      BP.set_motor_power(PORT_B, (power+((lightval - 50)/30)));
      BP.set_motor_power(PORT_C, (100-lightval)*power/30);
    }
    usleep(100000);
  }
 }                        
